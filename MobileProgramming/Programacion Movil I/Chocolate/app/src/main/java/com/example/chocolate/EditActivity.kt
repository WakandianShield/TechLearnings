...
import com.example.chocolate.databinding.ActivityEditBinding
...
class EditActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Solo el admin puede modificar registros
        ...
        // Toolbar universal: ocultar título del sistema y flecha de regreso
        ...
        if (ChocolateRepository.listaChocolates.isNotEmpty()) {
            mostrarChocolate(currentIndex)
        } else {
            Toast.makeText(this, "No hay chocolates para editar", Toast.LENGTH_SHORT).show()
            finish()
        }
        ...
        binding.btnSiguiente.setOnClickListener {
            if (currentIndex < ChocolateRepository.listaChocolates.size - 1) {
                currentIndex++
                mostrarChocolate(currentIndex)
            } else {
                Toast.makeText(this, "Es el último registro", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun mostrarChocolate(index: Int) {
        val chocolate = ChocolateRepository.listaChocolates[index]
        binding.etNombre.setText(chocolate.nombre)
        binding.etMarca.setText(chocolate.marca)
        binding.etPaisOrigen.setText(chocolate.paisOrigen)
        binding.etTelefono.setText(chocolate.telefonoContacto)
        binding.etPorcentajeCacao.setText(chocolate.porcentajeCacao.toString())

        setSpinnerValue(binding.spinnerPresentacion, chocolate.presentacion)
        setSpinnerValue(binding.spinnerTipoCacao, chocolate.tipoCacao)
        setSpinnerValue(binding.spinnerPerfilSabor, chocolate.perfilSabor)
        setSpinnerValue(binding.spinnerTipo, chocolate.tipo)
        setSpinnerValue(binding.spinnerPeso, chocolate.peso)
    }
    ...
    private fun guardarCambios() {
        ...
        val chocolateModificado = Chocolate(
            nombre = nombre,
            marca = marca,
            paisOrigen = pais,
        ...
        ChocolateRepository.listaChocolates[currentIndex] = chocolateModificado
