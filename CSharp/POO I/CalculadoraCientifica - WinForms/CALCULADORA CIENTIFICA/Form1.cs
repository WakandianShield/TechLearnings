...
namespace Calculator
{
    public partial class MainView : Form
    {
        ...
        private void on_click_equal(object sender, EventArgs e)
        {
            ...
            string operacion = $"{tbInput.Text} = {tbResult.Text}";
            historialOperaciones.Add(operacion);
            HISTORIAL.Text = operacion;
        ...
        private void btn_positive_negative_Click(object sender, EventArgs e)
        {
            ...
            // Verificamos si el texto tiene un número válido
            if (double.TryParse(currentText, out double number))
            {
                // Cambiar el signo del número
                double result = -number; // Cambia el signo
                tbInput.Text = result.ToString();
                tbInput.SelectionStart = tbInput.Text.Length; // Coloca el cursor al final
            }
            else
            {
        ...
        private void btn_logaritmo_natural_Click(object sender, EventArgs e)
        {
            caretIndex = tbInput.SelectionStart;
            string currentText = tbInput.Text;
            ...
            if (double.TryParse(currentText, out double number))
            {
                // Verificamos que el número sea mayor que 0 (el logaritmo de números negativos o cero no está definido)
                if (number > 0)
                {
                    double result = Math.Log(number); // Logaritmo natural
                    tbInput.Text = result.ToString();
                    tbInput.SelectionStart = tbInput.Text.Length; // Coloca el cursor al final
                }
                else
                {
                    MessageBox.Show("El logaritmo natural solo está definido para números mayores que 0.");
                }
            }
            else
            {
        ...
        private void btn_absolute_Click(object sender, EventArgs e)
        {
            ...
            if (double.TryParse(currentText, out double number))
            {
                ...
                double result = Math.Abs(number); // Valor absoluto
                tbInput.Text = result.ToString();
                tbInput.SelectionStart = tbInput.Text.Length; // Coloca el cursor al final
            }
            else
